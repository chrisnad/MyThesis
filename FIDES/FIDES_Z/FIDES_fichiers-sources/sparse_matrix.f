...
subroutine init_sparse(mat, nbl, nbc, nzmax, per)
    ...
    if (present(per) .and. per<1.d0 .and. per>0.d0) then; per1 = per; else; per1 = 1.; endif
    if (dime==2) then
        per1 = 7.0d0*nnt**(-0.85d0)   ! PTS  : deduire a partir des tests, jusqu'a 100000 ddl
    else
        per1 = 5.0d0*nnt**(-0.8d0)
    endif

    if (present(nzmax)) then; nzmax1 = nzmax; else; nzmax1 = nint(nbl*per1*nbc); endif
...
!
!> @brief
!> Procedure qui libere la place memoire reservee a une matrice stockee au format CSR
!
!> @details
!> #### DESCRIPTION:
!  Entrees :\n
...
!
!> @brief
!> Procedure qui permet d'assembler plus rapidement la matrice globale a partir des matrices elementaires
!
!> @details
!> #### DESCRIPTION:
!  Entrees :\n
...
!
!> @brief
!> Procedure qui permet d'ajouter des valeurs plus rapidement dans une matrice stockee au format CSR
!
!> @details
!> #### DESCRIPTION:
!  Entrees :\n
...
!
!> @brief
!> Procedure qui permet d'inserer plusieurs valeurs dans une matrice stockee au format CSR
!
!> @details
!> #### DESCRIPTION:
!  Entrees :\n
...
!>  - val : matrice contenant les valeurs a inserer\n\n
!
!  Sorties :\n
...
subroutine insert_values(A, indi, indj, val)
    ...
    !--------------------------------------------------------------------------------------
    !--- Variables in/out
    type(sparse), intent(inout) :: A                !< Matrice a modifier
    integer, dimension(:), intent(in) :: indi ,indj !< Indice de ligne et de colonne
    real*8, dimension(:), intent(in) :: val         !< Tableau de valeurs
