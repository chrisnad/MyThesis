...
!  Entrees :                                                                      !
!  - typ : le type de l'element                                                 !
!  - vprel : proprietes elementaires                                              !
!                                                                                 !
!  Sortie :                                                                       !
!  - vh : matrice de hooke                                                        !
!---------------------------------------------------------------------------------!
   subroutine elem_hooke(vh,typ,vprel)
	...
	implicit none
     ...
     real*8 :: ah, A, I, J, Db !caracteristiques poutre : section (A), inerties (I,J), diametre (Db)
     ...
     elseif (typ == 'BAR2') then
         eh = vprel(4)
         allocate(vh(1,1))         
         vh = eh
        
     ! Tenseur du module d'elasticite des elements poutres
     elseif ((typ == 'BEA2') .or. (typ == 'BEF2')) then
         eh=vprel(4);               ! terme EI
         if (typ == 'BEF2') then
             ah = vprel(5);         ! terme EA
             allocate(vh(2,2))
...
! Version de elem_kloce differentes adaptee à une variable globale unique


 subroutine elem_kloce2(ie,ta) 
...
!---------------------------------------------------------------------------!
   ...
   subroutine elem_info(elem, typelem)
      ...
      real*8, dimension(:,:), allocatable :: Q1, Q2, Q0    
      real*8, dimension(:), allocatable :: W1, W2 , W0
      integer :: ns      
      ...
      select case (typelem)
         ...
         case ('EJQ6')
          ...
          !  elem%Q(3,1) = Q0(2,1)
            ...
            elem%W=(/1.333333333,0.333333333,0.3333333333/)
            elem%Q(1,1) = 0.
            elem%Q(2,1) = 1.
            elem%Q(3,1) = -1.
         ...
         case ('EJT6')
            ...
            elem%Q(1:3,:) = Q0
            elem%Q(4,:) = (/ 0.3333333333333d0, 0.3333333333333d0 /)
         ...
         case ('EJT8')
            ...
            call init_vec(elem%W,5)
            call init_mat(elem%Q,5,2)

            elem%W(1:4) = W0
            elem%W(5) = 0.d0

            elem%Q(1:4,:) = Q0
            elem%Q(5,:) = (/ 0.0d0, 0.0d0 /)

            deallocate(W0,Q0)

            call init_mat(elem%ksin,8,3); call init_mat(elem%face,2,4)
            elem%ksin(1,:) = (/-1.d0,-1.d0, 0.d0/)
            elem%ksin(2,:) = (/ 1.d0,-1.d0, 0.d0/) 
            elem%ksin(3,:) = (/ 1.d0, 1.d0, 0.d0/)
            elem%ksin(4,:) = (/-1.d0, 1.d0, 0.d0/)
            elem%ksin(5,:) = (/-1.d0,-1.d0, 0.d0/)
            elem%ksin(6,:) = (/ 1.d0,-1.d0, 0.d0/) 
            elem%ksin(7,:) = (/ 1.d0, 1.d0, 0.d0/)
            elem%ksin(8,:) = (/-1.d0, 1.d0, 0.d0/)
         ...
         case ('MBI4')
            ...
            elem%order = 2
            call util_quad(Q0, W0, elem%order, 'GAUSS', 2)
            call init_vec(elem%W,size(W0)+1)
            call init_mat(elem%Q,size(W0)+1,2)
            elem%W(2:5) = W0
            elem%Q(2:5,:) = Q0

            ! Point central
            elem%W(1) = 0.d0
            elem%Q(1,:) = sum(elem%Q,1) / size(elem%Q,1)
         ...
         case ('MTH8')
            ...
            elem%ndln = 3
            elem%order = 2
            call util_quad(elem%Q, elem%W, elem%order, 'GAUSS', 3)
            
            call init_mat(elem%ksin,8,3); call init_mat(elem%face,6,4)
...
! Northwestern University
   ...
   select case (typel)
    ...
    case ('EJT8')
        !%%%%%%%%%%%%%%%%%%% EIGHT NODE HEXAEDRAL JOINT ELEMENT %%%%%%%%%%%%%%%%%%%
        !%
        !%              8
        !%            / | \
        !%          /   |    \
        ...
        !%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             ...
             N=0.25*(/(1.-xi)*(1.-eta),(1.+xi)*(1.-eta),(1.+xi)*(1.+eta),(1.-xi)*(1.+eta), &
                ...
                dNdxi(3,:) = 0.25*(/  1.+eta ,   1.+xi /)
                dNdxi(4,:) = 0.25*(/-(1.+eta),   1.-xi /)
                dNdxi(5,:) = 0.25*(/-(1.-eta), -(1.-xi)/)
                dNdxi(6,:) = 0.25*(/  1.-eta , -(1.+xi)/)
                dNdxi(7,:) = 0.25*(/  1.+eta ,   1.+xi /)
                dNdxi(8,:) = 0.25*(/-(1.+eta),   1.-xi /)
    ...
    case ('MTH8')
        !%%%%%%%%%%%%%%%%%%% CUBE8 EIGHT NODE HEXAEDRAL ELEMENT %%%%%%%%%%%%%%%%%%%
        !%
        !%              8
        !%            / | \
        !%          /   |    \
        ...
        dNdxi(1,:) = 0.125*(/-(1.-eta)*(1.-zeta),-(1.-xi)*(1.-zeta),-(1.-xi)*(1.-eta)/)
        dNdxi(2,:) = 0.125*(/ (1.-eta)*(1.-zeta),-(1.+xi)*(1.-zeta),-(1.+xi)*(1.-eta)/)
        dNdxi(3,:) = 0.125*(/ (1.+eta)*(1.-zeta), (1.+xi)*(1.-zeta),-(1.+xi)*(1.+eta)/)
        dNdxi(4,:) = 0.125*(/-(1.+eta)*(1.-zeta), (1.-xi)*(1.-zeta),-(1.-xi)*(1.+eta)/)
        dNdxi(5,:) = 0.125*(/-(1.-eta)*(1.+zeta),-(1.-xi)*(1.+zeta), (1.-xi)*(1.-eta)/)
        dNdxi(6,:) = 0.125*(/ (1.-eta)*(1.+zeta),-(1.+xi)*(1.+zeta), (1.+xi)*(1.-eta)/)
        dNdxi(7,:) = 0.125*(/ (1.+eta)*(1.+zeta), (1.+xi)*(1.+zeta), (1.+xi)*(1.+eta)/)
        dNdxi(8,:) = 0.125*(/-(1.+eta)*(1.+zeta), (1.-xi)*(1.+zeta), (1.-xi)*(1.+eta)/)
...
!-------------------------------------------------------------------------------!
   subroutine elem_B(N, B, detj, pt, e)
      ...
      !----- Aiguillage vers les routines de calcul -------
        ...
        select case (typel(1:2))
         ...
         case('MB')
                call elem_B_MB(N, B, detj, typel, noel, ndln, vcorn, dNdxi)
         case('MT')
                call elem_B_MT(N, B, detj, typel, noel, ndln, vcorn, dNdxi)
         case('EJ')
   ...
   contains
      ...
      !-------------------------------------------------------------------------!
           ...
           subroutine elem_B_MB(N, B, detj, typel, noel, ndln, vcorn, dNdxi)
      ...
      !-------------------------------------------------------------------------!
           ...
           subroutine elem_B_MT(N, B, detj, typel, noel, ndln, vcorn, dNdxi)
     ...
     subroutine elem_B_BE(N, B, detj, typel, noel, ndln, vcorn, dNdxi)
            ...
            real*8, dimension(:,:), allocatable :: NN, Nfem, R, Re, Re1, Re2, Bfem
