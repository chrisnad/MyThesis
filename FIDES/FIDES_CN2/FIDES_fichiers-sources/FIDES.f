...
!------------------- MPI (car MUMPS est ici compile en version parallele -----------------------------!
   ...
   integer :: compt
      ...
      integer, dimension(:), allocatable  :: ddlvalmax
      allocate(ddlvalmax(ndlt))
   ...
   !do while (.not.nconv)
        ...
        compt = compt +1
         ...
         !---------------------------------------------------------------------------!
              ...
              call init_vec(deboscill,nelt)
              call init_vec(detoscillp,nelt)
...
!%          !----------------------------------------------------------------------------------------!            %!
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%!
open(unit = 97, file = 'CN2.txt', status = 'REPLACE')
         do while (bpas)
                    ...
                    !----------------- Modification des proprietes mecaniques ----------------
                      if (ievtp) then
                          if (iter==1) then
                              vprelg1 = vprelg
                          !else
                              call interf_resichute(ipas) !,icalc_iter,iter) !--- chute de resistance pour fluage des elements d'interface
                          end if
                      end if
               ...
               !-------------------------------------------------------------------------!
                ...
                elseif ((npilot==3 .and. ipas .ge. 2).or.(npilot==4 .and. ipas .ge. 2)) then
                   ...
                   if(dime==2) then
                        ...
                        kdimp = (/dime*(1-1)+idir,dime*(7-1)+idir/)
                        vimpo=(/-1.d0,1.d0/)
                   else if(dime == 3) then
             ...
             elseif ((npilot==3).or.(npilot==4)) then   !--- Pilotage indirect
                 ipas = ipas+1
                 icalc_iter = 1
                 !------------- Sauvegarde des resultats a chaque pas de temps ----------!
...
!                     call ecriture(nomfichier,ipas-1)
             ...
             !---------------- Sauvegarde des resultats a chaque pas de temps --------------
             !if (isauvcalc==1 .and. iecritic==0) then
             if (isauvcalc==1) then
                 call evolglob()
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%!
    close(97)
         ...
         if (alea) then
              ...
              if (allocated(enerjba)) deallocate(enerjba)
              if (allocated(enerjba2)) deallocate(enerjba2)
         ...
         deallocate(detoscill,deboscill)
         if (interf==1) then
                do i=1,nelt
