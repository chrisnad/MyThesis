...
!-----------------------------------------------------------------------------------!
    ...
    subroutine elem_ke(vke,vre,ie,ndle,vprel,vdle)
       ...
       real*8 :: detj, wpla, wplael, poids, pdet, epais, rho, fvx, fvy, fvz, vfint(ndle), vfext(ndle)
...
!       end if
       ...
       do ipg = 1, npg
           ...
           !      wpla = dot_product(vnle(nc2,:),vn(:,1))
               ...
               !--- Cas particulier de la loi 25
!               if (int(vprel(1))==25 .and. ipg/=1) then
!                   vsig(1:size(vsi),ipg) = vsig(1:size(vsi),1)
!                   vnle(1:size(vnl),ipg) = vnle(1:size(vnl),1)
!                   vint(1:size(vin),ipg) = vint(1:size(vin),1)
!               end if

               !--- Cas particulier de la loi 27
               if (int(vprel(1))==27 .and. ipg/=1) then
                   vsig(1:size(vsi),ipg) = vsig(1:size(vsi),1)
                   vnle(1:size(vnl),ipg) = vnle(1:size(vnl),1)
                   vint(1:size(vin),ipg) = vint(1:size(vin),1)
               end if
           ...
           vke = vke + matmul(transpose(vb),matmul(vhep,vb))*pdet
