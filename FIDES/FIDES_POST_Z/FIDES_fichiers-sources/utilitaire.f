...
!
!> @brief
!> Lecture du nom du fichier de donnees et des informations utilisees pour la suite du calcul
!
!> @details
!> #### DESCRIPTION:
!>  Entrees :\n
...
subroutine lecture (nomfichier)
        ...
        if((CHAINE == 'ELEM').and.(err/=-1)) then
            ...
            do i = 1, ngrpe
                ...
                if (iloi == 1 .and. testmod) then
                    ...
                    ! 3D: Lecture de rho, E1, E2, E3, G12, G23, G31, nu12, nu23, nu31, RAB, Ea, epsl
    ...
    !--------------------------------------------------------------------------------------
    !--- Definition des forces de volume
...
!> #### DESCRIPTION:
!>  Entrees :\n
!>  - filemane : Nom du fichier de donnee\n\
...
!
!> @brief
!> Ecriture des distributions aleatoires de proprietes
!
!> @details
!> #### DESCRIPTION:
!>  A faire
...
!
!> @brief
!> Ecriture du fichier maillage contenant les fissures pour le traitement graphique
!> par le logiciel GID PTS - 03/04/2011
!
!> @details
!> #### DESCRIPTION:
!>  A faire
...
subroutine ecriture_fiss(nomfichier,nompas,Mcm)
    ...
    & iejq6, iejt6, ibar2, ibef3, imtp6, imth8
    ...
    ibar2 = 0 ; ibef2 = 0; ibef3 = 0; ibaf2 = 0; imtp6 = 0; imth8 = 0;
    ...
    !--------------------------------------------------------------------------------------
    !--- Trace des elements tridimensionnels (3D)
    if (dime==3) then
        ...
        if (imth8 == 1) then
            couc = couc + 1
            write(10,*) 'MESH "Layer', couc ,'" dimension 3 ElemType Hexahedral Nnode 8'
            write(10,*) '# Color 0.000000 0.000000 1.000000'
    ...
    !--------------------------------------------------------------------------------------
    !--- Trace des elements bidimensionnels (2D)
    elseif (dime==2) then
...
!
!> @brief
!> Fonction de calcul de l'ouverture moyenne d'une fissure
!
!> @details
!> #### DESCRIPTION:
!>  A faire
        ...
        do idir = 1, size(dirimpo)
            if (idir == dirimpo(1) .or. idir == dirimpo(2) .or. idir == dirimpo(3)) then
                if ((npilot == 1).or.(npilot == 2).or.(npilot==3)) then
                    ...
                    do i = 1,nnt
                        if (vfcg(infnoe(i)%dln(idir)) /=0.d0) then
                            kv(j) = infnoe(i)%dln(idir)
                            j = j +1
                        endif
                    enddo
...
!> Sauvegarde egalement l'heterogeneite du materiau dans le cas d'un calcul probabiliste
!
!> @details
!> #### DESCRIPTION:
!>  A faire
...
subroutine ecriture_resu(nomfichier,iopt)
    ...
    &                       fiss, ietat, inorm !, histetat1, histetat2
...
!
!> @brief
!> Routine de lecture du fichier binaire de résultats au dernier pas de calcul.
!
!> @details
!> #### DESCRIPTION:
!>  A faire
...
subroutine lecture_resu(nomfichier,iopt)
    ...
    if (option<=1) then
        ...
        if (interf==1) then
            !call init_mat(mbid,size(ietatpg,1),size(ietatpg,2))
...
!
!> @brief
!> Subroutine de calcul et d'ordonnancement des contraintes principales
!
!> @details
!> #### DESCRIPTION:
!>  En entree : mat0\n
...
!
!> @brief
!> Subroutine de constitution du vecteur knddl(nnt)
!
!> @details
!> #### DESCRIPTION:
