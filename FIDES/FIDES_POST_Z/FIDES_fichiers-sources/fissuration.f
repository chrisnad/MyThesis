...
subroutine fiss_sig(vsig,vnle,vh,vb,vnl0,vprel,vdle,iloi,ie)
    ...
    !--- Variables locales
    !--- Quantites globales
    real*8, dimension(size(vh,1),size(vh,2)) ::  Ps, Pe, Qs, Qe
    ...
    nc1 = size(vb,1)
    !fiss_changement_repere(V,n,iopt,typind)
    Ps = fiss_changement_repere(V,nc1,1,1)
    Qs = fiss_changement_repere(V,nc1,1,2)
    Pe = fiss_changement_repere(V,nc1,2,1)
    Qe = fiss_changement_repere(V,nc1,2,2)
    vsigi = 0.d0

    ! Dans le repere principal...
    vhloc = matmul(Ps,matmul(vh,Qe))
    vsigmloc = matmul(Ps,vsigm)
    vsigiloc = matmul(Ps,vsigi)
    vepsplloc = matmul(Pe,vepspl)
    ...
    vsigm  = matmul(Qs,vsigmloc)
    vepspl = matmul(Qe,vepsplloc)
...
!
!> @warning
!> Cette routine n'est pas appelée: elle a ete developpee pour la loi 12 mais le branchement
!> dans la routine elem_sig (contrainte.f) a ete deconnecte.
!
!> @todo
!> Routine a revoir car non utilisee !
!------------------------------------------------------------------------------------------------------
...
subroutine fiss_sig_endo(vsig,vnle,vin,vh,vb,vprel,vdle,iloi,ie)
    ...
    !--- Variables locales
    !--- Quantites globales
    real*8, dimension(size(vh,1),size(vh,2)) ::  Ps, Qs, vhpetit
    real*8, dimension(dime,dime) :: V  !, V0, VVsig
    real*8, dimension(dime) :: vsp  !, vsp0
    integer :: nc1, id  !, NROT
    ...
    select case(iloi)
        ...
        !----------------------------------------------------------------------------------
        !--- Loi d'endommagement (beton fissurant)
        case(15)
            ...
            !------------------------------------------------------------------------------
            !--- ACTIVATION DE L'EVOLUTION DE L'ENDOMMAGEMENT
            ...
            !--- Cas des elements deja fissures
            else
                if(ietat(ie) == 3) then
                    vcrit = .true.
                endif
...
subroutine fiss_crit(iloi,vprel,vsig,ie,vcrit,calcu,vdfdsig,vdgdsig)
    ...
    !--------------------------------------------------------------------------------------
    !--- Selection en fonction du type de loi
    select case(iloi)

        !----------------------------------------------------------------------------------
        !--- Loi de Tresca "orientee"
        case(12)
...
!
!> @warning
!> Fonction obsolete et fausse !!!
!
!> @todo
!> Supprimer cette fonction car fausse (differences entre deformations et contraintes non prise
!> en compte !!!)
!------------------------------------------------------------------------------------------------------
...
!> ### DESCRIPTION:
!> Definition de la matrice de changement de repere :
!> du repere global au repere principal et inversement\n\n
!
!------------------------------------------------------------------------------------------------------
...
subroutine fiss_pilot(imetpilo,vsol,vduI,vduII,dlam,alpham,MOTm,elefiss)
    ...
    real*8, dimension(:,:), allocatable :: vn, vb, vh, ksig, Ps
        ...
        case(1)
            ...
            if (fiss==1) then
                ...
                do ie = 1, nelt
                    ...
                    if ((typel=='MBT3'.or.typel=='MBQ4'.or.typel=='MBT6'.or.typel=='MTT4'.or. &
                        & typel=='MTP6'.or.typel=='MTH8') .and. (ietat(ie)==0)) then
                        ...
                        Ps = fiss_changement_repere(V,size(vb,1),1)
...
subroutine fiss_change_etat()
    ...
    &                       histetat1, fiss,kloce !iouver1, iouver2, histetat2,
    ...
    if (fiss==1) then
        do ie = 1, nelt                       ! Boucle sur les elements
            if (ietat(ie) /= 0) then          ! Element fissure
                ...
                if ((detoscill(ie)>=limite).and.(ietat(ie)==2)) then
                ...
                !--------------------------------------------------------------------------
                !--- Pour les elements fissures maintenus fermes (de force)
                if (ietat(ie)==-2) then
                    detoscill(ie) = detoscill(ie)+1
                    if (detoscill(ie)==limite) then
                        ! On libere l'element
                        detoscill(ie)=0
                        ietat(ie) = 2
                    endif
                    ...
                    !----------------------------------------------------------------------
                    !--- Actualisation de l'historique de changement d'etat
                    histetat1(ie)=ietat(ie)
                endif

                !--------------------------------------------------------------------------
                !--- Pour les elements fissures en ouverture ou en refermeture
                ! Ouverture
                ouv = fiss_ouv(ie)
                if ((ietat(ie)==1) .or. (ietat(ie)==2)) then
                    ...
                    !----------------------------------------------------------------------
                    !--- On teste ensuite le changement d'etat lié a la correction du deplacement a
                    !    l'iteration: on comptatbilise eventuellement une oscillation
...
subroutine fiss_init()
    ...
    if (ismember) then
        ...
        do while ((reply/='d') .and. (reply/='s'))
            !if(iprint>0) then
            !    print*, ' Bonjour ! Qu''est-ce qu''on va faire aujourd''hui pour s''amuser ?? '&
            !    & 'Smeared (s) ou depenalisation (d) ? (Enter = d) '
            !    read*, reply
            !endif
            !if (reply=='') reply = 'd'
