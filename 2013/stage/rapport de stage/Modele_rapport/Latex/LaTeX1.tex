...
d'effet d'échelle de Ba\v{z}ant. On confirme déjà l'existence de l'effet d'échelle sur des poutres de
...
et il existe un certain nombre d'études expérimentales et théoriques (\cite{rusch1962}; \cite{leonhardt1962}; \cite{kani1967}; \cite{taylor1972}; \cite{walsh1976}; \cite{chana1981}; \cite{reinhardt1981}; \cite{hillerborg1990}; eligehausen1992) qui confirment l'existence de celui-ci. Il ya deux aspects de l'effet d'échelle : statistique et
déterministe. Basés sur la mécanique de la rupture. Dans le passé, l'effet d'échelle a été
principalement traitée du point de vue statistique \cite{weibull1939}. Cependant, actuellement il ya suffisamment de preuves qui montrent que la
...
Selon \cite{bazant1984} l'effet d'échelle peut être approximativement décrite par la loi d'effet
...
\caption{Modèle de poutre entaillée, montage: flexion trois points}
\end{center}
\end{figure}
\\
...
$\frac{a_0}{d} = \frac{1}{6}$), réalisées à Northwestern University \cite{ba1990}, sont présentés. Dans
la même figure on compare ces résultats avec les résultats numériques \cite{eligehausen1992} et la loi d'effet d'échelle. La profondeur des poutres a été modifié dans une plage de taille
...
avec Pu étant la charge ultime. D'après la Fig. 1.2, un effet d'échelle significative est observé, la
...
\caption[Effet d'échelle en flexion trois points sur des poutres entaillées]{Effet d'échelle en flexion trois points sur des poutres entaillées ($a_0/d=1/6$)}
\end{center}
\end{figure}
\\
Les résultats des essais pour des échantillons non entaillées en béton ordinaire en flexion trois points sont représentés sur
la Fig. 1.3. La plage de hauteur de la poutre était jusqu'à $1000 mm$. Comme on peut le voir sur la
Fig. 1.3, les résultats des tests jusqu'à une profondeur de la poutre d'environ $d = 500 mm$
présentent un effet d'échelle significative en accord relativement avec la loi d'effet d'échelle. 
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.5]{figure3.png}
\caption{Effet d'échelle en flexion trois points sur des poutres non entaillées}
\end{center}
\end{figure}
\\
...
uniaxiale.
\clearpage
Dans la Fig. 1.4 la résistance nominale des poutres avec une taille constante d'entailles,
testés par \cite{tang1992} sont tracés en fonction de la hauteur. Comme dans le cas des
...
\caption[Effet d'échelle sur la contrainte nominale]{Effet d'échelle sur la contrainte nominale en flexion trois points sur des poutres entaillées (même taille d'entaille)}
\end{center}
\end{figure}
...
mesure des fractures / dommages %(\cite{lawler2001}; \cite{luo1993}; \cite{shah1995}). La méthode de corrélation d'images a été fréquemment
utilisée pour l'étude du mode d'ouverture de la rupture de matériaux quasi-fragiles comme le
béton. Dans les travaux de %\cite{choi1997}, la précision de mesure de
...
\caption[Correlation d'images]{(a) Zoom sur un motif moucheté. Le modèle speckle est d'environ $15\times 20$ pixels. (b) Représentation schématique de la déformé et l'image de référence situés sur une photo instantanée, \cite{alam2012}.
\end{center}
\end{figure}
\\
...
fonctions de forme d'une discrétisation par éléments finis, \cite{hild2008}.
