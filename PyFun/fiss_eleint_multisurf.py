...
filename = "poutre_eleinterf_pl_2"
directory = "/home/christian/Documents/FIDES/FIDES_POST/FIDES_resultats/" + filename + "/10"
...
for I in surf:
	...
	for i in [j for j in coor if ((coor[j][1] <= I+0.6*0.00475) and (coor[j][1] >= I-0.6*0.00475))]:		#0.00475 is mesh dependant (margin for the surface ordinate)
		noeuds_surf[i] = coor[i]
		
	unique_noeuds = [list(x) for x in set(tuple(x) for x in noeuds_surf.values())]     #unique lists in a list of lists
	
	i = 1
	for j in unique_noeuds:
		unique_nodes[i] = []
		for k in noeuds_surf:
			if noeuds_surf[k] == j:
				unique_nodes[i].append(k)
		i = i+1
	...
	for j in unique_nodes:
		dl = []
		for p1,p2 in itertools.combinations(unique_nodes[j], 2):
			dl.append([calc_dist(x,y) for x,y in zip(noeuds_surf_pos[p1], noeuds_surf_pos[p2])])
		delta[noeuds_surf[p1][0]] = [max(sublist) for sublist in itertools.izip(*dl)]
